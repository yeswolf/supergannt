...
final class MppOleWriter {
  ...
  static void write(ProjectFile project, byte[] templateBytes, byte[] mspdiXml, OutputStream out)
      throws Exception {
  ...
  private static void embedSgantt(DirectoryEntry root, byte[] xml) throws Exception {
    ...
    if (root.hasEntry(Convert.SGANTT_DIR)) {
      Entry e = root.getEntry(Convert.SGANTT_DIR);
      if (e instanceof DirectoryEntry) {
        sgantt = (DirectoryEntry) e;
        deleteChildren(sgantt);
      } else {
        e.delete();
        sgantt = root.createDirectory(Convert.SGANTT_DIR);
      }
    } else {
      sgantt = root.createDirectory(Convert.SGANTT_DIR);
    }
    sgantt.createDocument(Convert.MSPDI_STREAM, new ByteArrayInputStream(xml));
    sgantt.createDocument(
        Convert.MANIFEST_STREAM,
        new ByteArrayInputStream(Convert.MANIFEST_JSON.getBytes(StandardCharsets.UTF_8)));
  }
  ...
  private record VarEntry(int uid, int type, int offset) {}
}
